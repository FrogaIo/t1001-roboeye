...
## Готовый промпт для другого агента

Скопировать агенту целиком:

```text
Ты работаешь в репозитории:
/Users/roman/Documents/GitHub/t1001-roboeye

Твоя задача — не написать ещё один план, а реализовать рабочий Godot-симулятор
RoboEye. Сначала полностью прочитай README.md, GODOT_PLAN.md и DEPTH_PLAN.md,
затем изучи текущий код и git log. В репозитории могли появиться изменения
другого агента, особенно по Depth Anything: сохрани их, не откатывай и не
перезаписывай. Файл camera_test.py принадлежит пользователю — не редактируй и не
добавляй его в коммиты.

Целевой результат:

1. Создай Godot 4.7.x Standard-проект в godot/.
2. Собери простой 3D-склад из встроенных примитивов: пол, стены, свет, цветные
   коробки, стол, стул и неизвестное препятствие. Не скачивай asset packs.
3. Сделай дифференциального робота на CharacterBody3D с физическими
   столкновениями, CameraAnchor и ручным управлением WASD/gamepad.
4. Добавь отдельный SubViewport 640x360 и Camera3D, совпадающую с
   CameraAnchor. Каждые ~333 мс получай RGB Image после
   RenderingServer.frame_post_draw, кодируй JPEG quality 0.68 и отправляй в
   существующий Python WebSocket.
5. Для локальной симуляции используй ws://127.0.0.1:8000/ws. Протокол уже
   существует: сначала text JSON
   {"type":"orientation","pitch_degrees":<угол камеры>}, затем binary JPEG;
   в ответ приходит text JSON с state, reason, route, lanes, detections,
   corridor, inference_ms и pitch_degrees.
6. Не отправляй следующий кадр до ответа на предыдущий. Регулярно вызывай
   WebSocketPeer.poll(), обрабатывай disconnect и автоматически переподключайся
   с разумной задержкой.
7. Реализуй режимы MANUAL и ASSIST. В MANUAL vision только отображается. В
   ASSIST команда джойстика основная, но при STOP движение вперёд запрещается,
   задний ход остаётся доступен, а LEFT/RIGHT вызывает поворот на месте в
   выбранную сторону. BLOCKED полностью останавливает робота. Если vision-ответ
   старше 1 секунды или соединение потеряно, движение вперёд запрещено.
8. Зафиксируй выбранную сторону объезда минимум на 1 секунду, чтобы LEFT/RIGHT
   не дрожали. AUTO делай только после того, как ASSIST надёжно работает.
9. Сделай HUD: большой вид сцены от третьего лица, inset камеры робота, статус
   соединения, MANUAL/ASSIST, GO/STOP, reason, route, ping, inference, три
   полосы, RESET и RANDOMIZE. Поверх inset нарисуй detections и corridor из
   нормализованных координат ответа — геометрия должна совпадать с Python.
10. Добавь минимум сценарии empty, center_box, left_right_choice, blocked,
    unknown_object и colored_cubes. RESET должен воспроизводимо возвращать
    выбранный сценарий.

Критически важное правило честности: steering может использовать только RGB,
JSON-ответ RoboEye, ввод пользователя и собственную скорость/heading робота.
Запрещено использовать позиции препятствий, RayCast3D, collision contacts или
настоящий depth buffer Godot для выбора направления. Collision/ground-truth
разрешены только для metrics и тестовой оценки.

Порядок работы:

- сначала проверь git status и наличие команды godot;
- реализуй этап A из GODOT_PLAN.md и запусти сцену;
- затем этап B и сквозной RGB -> Python -> JSON тест;
- затем этап C и три ручных сценария ASSIST;
- только потом занимайся полировкой HUD, AUTO и monocular depth;
- не оставляй симуляцию в состоянии, где она существует только как набор
  файлов: реально запусти Godot, проверь output/errors и весь поток, если Godot
  доступен;
- если Godot не установлен, создай валидный проект и сцены, выполни доступные
  статические проверки, но честно сообщи, что runtime не проверен. Не утверждай,
  что всё работает без запуска;
- не ломай телефонный режим и существующую Mac-панель;
- обнови README точными командами запуска симуляции;
- добавь godot/.godot, export, logs и screenshots в .gitignore.

Делай небольшие проверяемые коммиты после каждого законченного этапа:
...
Перед каждым коммитом проверяй git diff --check и не включай посторонние файлы.
В конце дай пользователю короткий отчёт: что реально работает, что проверено,
команды запуска, управление, известные ограничения и список созданных коммитов.
Не останавливайся после анализа или плана — реализуй максимально полный рабочий
MVP, пока остаются безопасные действия в рамках задачи.
```
